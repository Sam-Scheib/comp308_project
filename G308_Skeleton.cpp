 ...
 */
/**
 * Read an amc file and prepare the system for playing that specific
 * file, returns READ_SUCCESS if it read the file
 * or READ_FAILURE if it could not read/finish reading the file
 */
void Skeleton::readAMCFile(char* filename) {
	char buff [60];
	char name [60];
	int frameNumber = 1;
	//attempt file load
	...
	if (file == NULL) {
		printf("Failed to open file %s for frame count\n", filename);
		exit(READ_FAILURE);
	}
	printf("Finding frame size\n");
	...
	//There are (NUM_BONES_IN_ASF_FILE - 2) moving bones(2 dummy bones, lhipjoint and rhipjoint)
	//read in a line from the file
	m_numFrames = (n-3)/(numBones-1);
	printf("File has %d frames\n", m_numFrames);
	frame_ctrl = new FrameController(m_numFrames);
	//set up Posture and load default values
	m_Postures = new Posture[m_numFrames];
	setDefaultPostures();

	//re-open file for use in actual reading
	file = fopen(filename, "r");
	if (file == NULL) {
		printf("Failed to open file %s for reading in frames\n", filename);
		exit(READ_FAILURE);
	}
	printf("Reading in frames to Animation\n");
	while((fgets(buff, BUFF_SIZE, file)) != NULL ) {
		//set up the bone change struct for this bone
		//intialise all values to simplify things later
		...
		//see if we can read in a framenumber, which is used for storing items
		//in the m_Postures array
		int result = sscanf(buff, "%d", &frameNumber);
		if (result==1) {
			continue;
		}

		//first get the name of this bone and store it in the name pointer
		//as well as attempting to get all other info about rotations and
		//translations in one go!
		float one, two, three, four, five, six = 0;
		result = sscanf(buff, "%s %f %f %f %f %f %f", name, &one, &two, &three, &four, &five, &six);
		if (result < 1) {
			printf("unknown animation string: %s\nin file: %s\n", buff, filename);
			exit(READ_FAILURE);
		}

		//now we have to find that bone amongst our skeleton file
		//and get the dof of that bone
		bone* bone= findBone(name);
		//now we have to use the dof value to determine how we will
		//assign values
		switch(bone->dof){
		case 1 :// X
			m_Postures[frameNumber].bone_rotation[bone->id].x = one;
			break;

		case 2 :// Y
			m_Postures[frameNumber].bone_rotation[bone->id].y = one;
			break;

		case 3 :// X + Y
			m_Postures[frameNumber].bone_rotation[bone->id].x = one;
			m_Postures[frameNumber].bone_rotation[bone->id].y = two;
			break;
		...
		case 5 :// X + Z
			m_Postures[frameNumber].bone_rotation[bone->id].x = one;
			m_Postures[frameNumber].bone_rotation[bone->id].z = two;
			break;

		case 6 :// Y + Z
			m_Postures[frameNumber].bone_rotation[bone->id].z = one;
			m_Postures[frameNumber].bone_rotation[bone->id].z = two;
			break;
		...
		case 7 :// X + Y + Z
			m_Postures[frameNumber].bone_rotation[bone->id].x = one;
			m_Postures[frameNumber].bone_rotation[bone->id].y = two;
			m_Postures[frameNumber].bone_rotation[bone->id].z = three;
			break;

		case 8 :// case root!
			m_Postures[frameNumber].root_pos.x = one;
			m_Postures[frameNumber].root_pos.y = two;
			m_Postures[frameNumber].root_pos.z = three;
			m_Postures[frameNumber].bone_rotation[bone->id].x = four;
			m_Postures[frameNumber].bone_rotation[bone->id].y = five;
			m_Postures[frameNumber].bone_rotation[bone->id].z = six;
			break;

		default :
			printf("warning, the dof (%d) for bone %s, didn't match a known value\n", bone->dof, name);
			exit(READ_FAILURE);
		}
		G308_Point t;
		t.x = 1;
		t.y = 0;
		t.z = 0;
		const G308_Point f = (const G308_Point) t;
		const quaternion q_x(m_Postures[frameNumber].bone_rotation[bone->id].x, f);
		t.x = 0;
		t.y = 1;
		const quaternion q_y(m_Postures[frameNumber].bone_rotation[bone->id].y, t);
		t.y = 0;
		t.z = 1;
		const quaternion q_z(m_Postures[frameNumber].bone_rotation[bone->id].z,t);

		const quaternion complete = q_x * q_y * q_z;
		m_Postures[frameNumber].bone_rot_q[bone->id] = complete;
	}
	printf("Finished reading animation file\n");
}
